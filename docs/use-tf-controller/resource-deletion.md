# Resource Deletion Dependencies in Tofu Controller

This document discusses potential difficulties you may encounter when deleting Terraform resources 
through the Tofu Controller and the necessary components to facilitate a smooth deletion process.
...
During the deletion process, the Tofu Controller uses these source files to conduct a re-planning operation. 
...
As a result, the Tofu Controller cannot locate the resource state, 
...
the finalizer of the Tofu Controller.
...
As it skips the finalizer of the Tofu controller, any cleanup the
...
the Tofu Controller leverages Secrets and ConfigMaps to generate a complete source before planning. 
...
Should any of these components be missing or misconfigured, the Tofu Controller may fail to generate an accurate deletion plan, 
...
As of now, we are actively working to address these limitations in the Tofu Controller. 
We appreciate your patience and welcome any feedback to help enhance the Tofu Controller's performance.
